...
export default function App(){
  ...
  useEffect(()=>{
    ...
    channel.on("broadcast", { event: "challenge" }, ({payload})=>{
      if(payload.toId===myId){ setIncomingChallenge({ fromId:payload.fromId, fromName:payload.fromName, matchId:payload.matchId, fromLine:payload.fromLine }); }
    });
    channel.on("broadcast", { event: "challenge_response" }, ({payload})=>{
      if(payload.toId===myId){
        if(payload.accepted){
          setOutgoingChallenge(null);
          // Opponent accepted → both enter the shared match room. Challenger is host.
  ...
  function sendChallenge(toId,toName){
    ...
    const myName=profileName || (session.user.email||"Fighter").split("@")[0];
    const matchId=`${session.user.id.slice(0,8)}-${Date.now()}`; // unique match id, generated by challenger
    clubChannelRef.current.send({ type:"broadcast", event:"challenge", payload:{ fromId:session.user.id, fromName:myName, fromLine:warriorKey||"mongol", toId, matchId } });
    setOutgoingChallenge({ toId, toName, matchId });
  }
  ...
  // Join the dedicated realtime channel for an active match and open the live duel screen.
  function enterLiveMatch({ matchId, opponentId, opponentName, opponentLine, iAmHost }){
    ...
    ch.subscribe(async(status)=>{
      if(status==="SUBSCRIBED"){ await ch.track({ id: session.user.id }); }
    });
...
const Z={
  ...
  liveTag:{textAlign:"center",fontFamily:"'Bebas Neue', sans-serif",fontSize:20,color:"#B33A3A",letterSpacing:1,marginBottom:16},
  liveVs:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 8px",marginBottom:20},
  liveStatus:{background:"#1D232D",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"18px 16px",textAlign:"center",marginTop:"auto",marginBottom:12},
  liveStatusMain:{fontFamily:"'Bebas Neue', sans-serif",fontSize:24,letterSpacing:0.5,marginBottom:6},
  liveStatusSub:{fontSize:12,lineHeight:1.5,color:"#8B95A3"},
};
